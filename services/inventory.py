...
class InventoryService:
    ...
    def deduct_stock(self, medicine_name: str, quantity: int) -> Inventory:
        ...
        Raises:
            InsufficientStockError: If current stock can no longer cover the
                requested quantity (e.g. it changed since the item was added
                to the cart).
        """
        item = self.get_by_name(medicine_name)
        if quantity > item.Stock_Available:
            raise InsufficientStockError(medicine_name, quantity, item.Stock_Available)
